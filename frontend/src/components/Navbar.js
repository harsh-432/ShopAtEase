...
export default function Navbar(props) {
  ...
  const handleLogout = async (event) => {
    try {
      let response = await fetch(`${process.env.REACT_APP_URI}/api/v1/logout`, {
        method: "get",
        headers: {
      ...
      <nav className="navbar">
        <div className="s1">
          ...
          <form className="searchbar" onSubmit={submitHandler}>
            <input
              type="text"
              placeholder="   Search for products or shop"
              className="search"
              value={searchKey}
          ...
          {!localStorage.getItem("authToken") ? (
            ...
            <div>
              <div className="btn">
                <Link className="" to="/my-cart">
                  <span>My Cart </span>
                </Link>
                <Badge pill bf="danger">
                  {noOfItems()}
                </Badge>
              </div>
